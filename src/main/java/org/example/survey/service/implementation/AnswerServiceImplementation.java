...
public class AnswerServiceImplementation  implements AnswerService {
    ...
    @Override
    public Optional<AnswerDto> updateAnswer(Long questionId, Long answerId, AnswerDto answerDto) throws AnswerNotFoundException {
        Question  questionFromDB = questionRepository.findById(questionId).get();
    ...
    @Override
    public Optional<AnswerDto> updateAnswerPartially(Long questionId, Long answerId, AnswerDto answerDto) throws AnswerNotFoundException {
        Question  questionFromDB = questionRepository.findById(questionId).get();
        Optional<Answer>answerFromDB = answerRepository.findById(answerId);
        if(answerFromDB.isPresent()){
            answerFromDB.get().setQuestion(questionFromDB);
